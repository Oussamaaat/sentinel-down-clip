...
os.environ['GDAL_DATA'] = 'sdc_env\Lib\site-packages\osgeo\data\gdal'
os.environ['PROJ_LIB'] = 'sdc_env\Lib\site-packages\osgeo\data\proj'
...
def sentinel2_process(args, zf, mini, maxi):
    ...
    for fname in filelist:
        if("GRANULE" in fname and "IMG_DATA" in fname and ".jp2" in fname):
            ...
            infile = os.path.join("tmp",fname)
            outfile = os.path.join(args.dest, root_fname+".tif")
            #cmd = ['sdc_env\Lib\site-packages\osgeo\gdalwarp', "-t_srs", "EPSG:4326", "-te", str(mini[0]), str(mini[1]), str(maxi[0]), str(maxi[1]), "-ts", "1024", "1024", infile, outfile]
            cmd = ['sdc_env\Lib\site-packages\osgeo\gdalwarp', "-t_srs", "EPSG:4326", "-te", str(mini[0]), str(mini[1]), str(maxi[0]), str(maxi[1]), infile, outfile]
            subprocess.call(cmd)
